...
| **Pivoted Phase 4.6 — FastAPI service → Cloud Run Job + CLI orchestrator** (2026-05-24) | Three problems with the original FastAPI + `BackgroundTasks` plan surfaced during design review: (1) `BackgroundTasks` runs in the same process *after* the HTTP response — Cloud Run can kill the container when the request closes, and a 1-hour transcription job won't survive that; (2) in-memory job state dies on cold start or scale-to-zero, so a POST → poll workflow has a race against eviction; (3) the agent never *triggers* ingestion — the user does, manually, when a new recording lands in Drive. Cloud Run **Jobs** (distinct from Services) are purpose-built for this batch-execution shape: no HTTP, no in-memory state, exit code is the durable signal. New plan: `ingestion/orchestrator.py` CLI that's the `ENTRYPOINT` of `Dockerfile.ingestion`, runnable both locally (`python -m ingestion.orchestrator`) and as a Cloud Run Job (`gcloud run jobs execute ingest-recordings`). Idempotency via a GCS `.indexed` sentinel written only after a successful Pinecone upsert — survives container teardown, distinguishes complete from half-completed runs in a way that Pinecone metadata alone cannot. |
