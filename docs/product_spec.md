...
| Translation | Swami Sivananda's, from the original translator text (§7.1), named in every lesson. Confirmed for v1 on 2026-09-13 (content spec §2.6); open question 6 governs v2 public use |
...
| 6 | Default English translation for v2 public use. Sivananda is confirmed for v1. Candidates for v2: Besant (1895, no rights doubt, leading) with Sivananda as alternate; the three later translators are review-appendix only. **Deliberately deferred to the v2 product spec**, to be decided with v1 experience and reviewer feedback in hand. Needs a rights check, not a taste call. | Udaya, v2 spec | Not for v1; yes before v2 |
